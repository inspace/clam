...
def find_records(name, rtype, address):
    ...
    if not record_list or rtype != RTYPE_A or record_list[0].RTYPE == RTYPE_CNAME or record_list[0].ipadrs != EMPTY:
        return record_list
...
def persist(key, value):
    r.sadd(key, value) # set add for multiple responding recursive resolvers
    #r.expire(key, 20)
...
def udp_proc(data_struc, ssock):
    (pstr, adrs) = data_struc
...
def main():
    ...
    with open('dns.txt') as f:
        for line in f:
            ...
            if record_type == 'SOA':
                add_record(host, RecordSOA(ttl, 'dns1.registrar-servers.com', 'root@ipgeoloc.com', None, 3600, 1800, 20000, 3600))
            elif record_type == 'A':
                ip = chunks[3]
                add_record(host, RecordA(ttl, ip))
            elif record_type == 'CNAME':
                other_host = chunks[3]
                add_record(host, RecordCNAME(ttl, other_host))
            else:
